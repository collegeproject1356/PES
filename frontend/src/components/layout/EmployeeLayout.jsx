...
import { LayoutDashboard, Target, Edit3, MessageSquare, LogOut } from 'lucide-react';
...
const EmployeeLayout = () => {
  ...
  const menuItems = [
    { path: '/employee', icon: <LayoutDashboard size={20} />, label: 'My Dashboard' },
    { path: '/employee/goals', icon: <Target size={20} />, label: 'Assigned Goals' },
    { path: '/employee/self-evaluation', icon: <Edit3 size={20} />, label: 'Self Evaluation' },
    { path: '/employee/feedback', icon: <MessageSquare size={20} />, label: 'Performance Feedback' },
  ];
  ...
  return (
    <div className="flex h-screen bg-[#FAFAFB] overflow-hidden">
      ...
      >
        ...
        <div className="px-6 pb-6 border-b border-gray-100/50 flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center font-bold text-xl text-indigo-600 shadow-sm">
            {user?.name?.charAt(0) || 'E'}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-gray-900 truncate">{user?.name || 'Employee'}</p>
            <p className="text-xs text-indigo-600/80 font-medium truncate">{user?.role}</p>
          </div>
        </div>
